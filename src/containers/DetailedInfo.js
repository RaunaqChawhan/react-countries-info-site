...
const DetailedInfo = ({match, countries}) => {
    ...
    const [ filteredCountryDetails ] = countries.filter(country => country.name === match.params.country)

    const { flag, name, nativeName, population, region, subregion, capital, topLevelDomain, currencies, languages, borders } = filteredCountryDetails;
    ...
    let borderCountries = borders.map(border => countries.filter(country => country.alpha3Code === border)); // extract the border country names from border country codes
    borderCountries = [].concat(...borderCountries);
    const currencyNames = currencies.map(currency => currency.name);    // extract the currency name
    const languagesName = languages.map(language => language.name); // extract the name of languages
